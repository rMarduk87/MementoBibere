...
class StringHelper(var mContext: Context, var act: Activity) {
    ...
    fun getHtmlData(data: String): String {
        ...
        return "<html>$head<body style=\"text-align:justify\">$data</body></html>"
    }
    ...
    fun getArrayListFromCommaSeparatedString(comma_string: String): ArrayList<String> {
        ...
        val str_arr = comma_string.split(",".toRegex()).dropLastWhile { it.isEmpty() }
            .toTypedArray()
    ...
    fun getArrayListFromCommaSeparatedString(
        ...
        val arr = ArrayList<String>()
        val str_arr = comma_string.split(sign.toRegex()).dropLastWhile { it.isEmpty() }
            .toTypedArray()
        for (k in str_arr.indices) arr.add(str_arr[k])
        return arr
    }
    ...
    fun getArrayFromCommaSeparatedString(comma_string: String): Array<String> {
        return comma_string.split(",".toRegex()).dropLastWhile { it.isEmpty() }
            .toTypedArray<String>()
    }

    fun getArrayFromCommaSeparatedString(
        comma_string: String,
        sign: String
    ): Array<String> {
        return comma_string.split(sign.toRegex()).dropLastWhile { it.isEmpty() }
            .toTypedArray<String>()
    }
    ...
    fun get_arraylist(id: Int): ArrayList<String> {
        return ArrayList(Arrays.asList(*mContext.resources.getStringArray(id)))
    }
    ...
    fun is_not_a_number(height: String): Boolean = !height
    .removePrefix("-")
    .removePrefix("+")
    .all { it in '0'..'9' }
}
